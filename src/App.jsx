...
function App() {
  ...
  const bgImage = useMemo(() => {
    if (!weather) return clearGif;
    const c = weather.current.condition.text.toLowerCase();
    const isDay = weather.current.is_day === 1;
    if (c.includes("rain") || c.includes("drizzle"))                return rainGif;
    if (c.includes("cloud") || c.includes("overcast"))              return cloudyGif;
    if ((c.includes("sunny") || c.includes("clear")) && isDay)      return sunnyGif;
    return clearGif;
  }, [weather]);

  const bgBrightness = useMemo(() =>
    weather && weather.current.temp_c > 16 ? "brightness(1.05)" : "brightness(0.85)"
  , [weather]);
          ...
          <div style={{ minHeight: "100%", display: "flex", flexDirection: "column" }}>
            ...
            {error && (
              <div style={{ maxWidth: 520, margin: "0 auto 16px", width: "90%",
                            background: "rgba(180,30,30,0.5)", border: "1px solid rgba(255,100,100,0.4)",
              ...
              <div className="fade-up" style={{ flex: 1, maxWidth: 960, margin: "0 auto",
                ...
                <div style={{ textAlign: "center", marginBottom: 24 }}>
                  <div style={{ display: "inline-block", background: "rgba(255,255,255,0.2)",
                                borderRadius: 28, padding: "24px 52px",
                  ...
                  {cards.map(({ icon, label, value, sub }) => (
                    <div key={label} className="card">
                      ...
                      <p style={{ color: "rgba(255,255,255,0.95)", fontSize: "0.72rem",
                                  ...
                                  fontWeight: 700, marginBottom: 6 }}>{label}</p>
                      <p style={{ color: "white", fontSize: "1.55rem", fontWeight: 900,
                                  textShadow: "0 2px 6px rgba(0,0,0,0.4)", lineHeight: 1.1 }}>{value}</p>
                      {sub && <p style={{ color: "#fde68a", fontSize: "0.8rem", fontWeight: 700, marginTop: 4 }}>{sub}</p>}
                    </div>
                  ))}
                </div>
              </div>
